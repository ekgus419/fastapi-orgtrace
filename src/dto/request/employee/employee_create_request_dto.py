...
class EmployeeCreateRequestDto(BaseModel):
    ...
    name: str                       = Field(..., max_length=100, description="직원 이름")
    email: EmailStr
    phone_number: str               = Field(..., max_length=20, description="핸드폰 번호")
    extension_number: str           = Field(..., max_length=10, description="내선 번호")
    hire_date: date                 = Field(default_factory=date.today, description="입사일")
    birth_date: date                = Field(default_factory=date.today, description="생일")
